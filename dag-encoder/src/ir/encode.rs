use crate::{
    Graph, Instruction, Operand, INSTRUCTION_BITS, INTEGER_BASE, SHORT_INTEGER_BASE,
    SYMBOL_SEPARATOR, SYMBOL_TERMINATOR,
};
use alloc::{string::String, vec, vec::Vec};

/// Encodes a program.
pub fn encode(program: &Graph) -> Vec<u8> {
    let mut codes = vec![];
...
mod tests {
    ...
    fn encode_and_decode(program: &Graph) {
        assert_eq!(&decode(&encode(program)).unwrap(), program);
    }

    #[test]
    fn encode_nothing() {
        encode_and_decode(&Graph::new(vec![], vec![]));
    }

    #[test]
    fn encode_symbol() {
        encode_and_decode(&Graph::new(vec!["foo".into()], vec![]));
    }

    #[test]
    fn encode_symbols() {
        encode_and_decode(&Graph::new(vec!["foo".into(), "bar".into()], vec![]));
    }
    ...
    fn encode_empty_symbol() {
        encode_and_decode(&Graph::new(vec!["".into()], vec![]));
    }

    #[test]
    fn encode_empty_symbols() {
        encode_and_decode(&Graph::new(vec!["".into(), "".into()], vec![]));
    }

    #[test]
    fn encode_empty_symbols_and_symbol() {
        encode_and_decode(&Graph::new(
    ...
    #[test]
    fn encode_nop() {
        encode_and_decode(&Graph::new(vec![], vec![Instruction::Nop(42)]));
    }
